...
export function CustomEntry16SeatsControlPanel({
    userStreamRef,
    stopSelfStream,
}: EntryPanelProp) {
    return (
...
function EnterButton({ stopSelfStream }: PanelButtonProp) {
    const { roomInfo, selectedSeatNo, checkVacancy, enterRoom } =
        useCustomEntryContext();
            ...
            {selectedSeatNo === 0 ? (
                ...
                <EnterButton__Enable
                    onClick={async function () {
                        if (await checkVacancy(roomInfo.id, selectedSeatNo)) {
                            stopSelfStream();
                            enterRoom(roomInfo.id, selectedSeatNo);
                        }
                    }}
                >
                    참여하기
